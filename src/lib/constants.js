...
export const BADGES = [
  ...
  // Medical badges
  { id: "anatomy_master", icon: "🦴", label: "Anatomy Master",   desc: "Complete 50 Anatomy questions correctly",  check: (s)       => s.totalCorrect >= 50 },
  { id: "pharma_pro",     icon: "💊", label: "Pharma Pro",        desc: "Complete 50 Pharmacology questions",       check: (s)       => s.totalCorrect >= 50 },
  { id: "clinical_reasoner", icon: "🧠", label: "Clinical Reasoner", desc: "Solve 10 clinical case simulations",     check: (s)       => s.sessions >= 10 },
  { id: "osce_ready",    icon: "🏥", label: "OSCE Ready",       desc: "Complete 5 OSCE practice stations",        check: (s)       => s.sessions >= 5 },
  { id: "first_diagnosis", icon: "🔍", label: "First Diagnosis", desc: "Complete your first clinical case",        check: (s)       => s.sessions >= 1 },
  { id: "code_blue",     icon: "🚨", label: "Code Blue",        desc: "Score 100% on an emergency medicine quiz", check: (s, h)    => h.some(x => x.score === x.total && x.total > 0 && x.mode === "exam"), rare: true },

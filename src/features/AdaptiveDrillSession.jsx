...
import { X, ChevronRight, ChevronLeft, RotateCcw, Target, Flame, Zap } from "lucide-react";
...
export default function AdaptiveDrillSession({ subject, resourceIds, onBack, onStreakUpdate, onXpUpdate }) {
  ...
  const combo = useComboStreak("adaptive");
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API_BASE}/api/resources/fsrs/due-mcqs?${params.toString()}`, { headers: getAuthHeaders() }).then((r) => r.json()),
      fetch(`${API_BASE}/api/resources/fsrs/weak-topics?${params.toString()}`, { headers: getAuthHeaders() }).then((r) => r.json()),
    ])
      .then(([dueData, topicsData]) => {
        setWeakTopics(topicsData.topics || []);
        const enriched = (dueData.items || [])
          .filter((i) => i.mcq)
  ...
  if (loading) {
    return (
      <div style={{ ...fullscreenStyle, alignItems: "center", justifyContent: "center" }}>
        <div style={{ color: "#7b82b8", fontSize: "15px" }}>Analyzing weak topics...</div>
      </div>
    );
  }
  ...
  if (items.length === 0) {
    return (
      <div style={{ ...fullscreenStyle, alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ fontSize: "15px", color: "#7b82b8", marginBottom: "16px" }}>No weak items found in scope!</div>
    ...
    return (
      <div style={{ ...fullscreenStyle, overflowY: "auto" }}>
        <div style={{ maxWidth: "500px", margin: "0 auto", padding: "60px 20px 40px", textAlign: "center" }}>
          <div style={{ fontSize: 40, marginBottom: 12 }}>{pct >= 70 ? "🎯" : "📚"}</div>
          <h2 style={{ color: "#c5c9e8", fontSize: 22, fontWeight: 800, marginBottom: 20 }}>Adaptive Drill Complete!</h2>
          <div style={{ display: "flex", gap: 12, justifyContent: "center", marginBottom: 24, flexWrap: "wrap" }}>
            <div style={{ background: "#0d0f20", border: "0.5px solid #1e2245", borderRadius: "14px", padding: "14px 22px", textAlign: "center" }}>
              ...
              <div style={{ fontSize: 10, color: "#5a6090", textTransform: "uppercase", letterSpacing: "0.05em" }}>Drilled</div>
            </div>
            <div style={{ background: "#0d0f20", border: "0.5px solid #1e2245", borderRadius: "14px", padding: "14px 22px", textAlign: "center" }}>
          ...
          {weakTopics.length > 0 && (
            <div style={{ marginBottom: 20, textAlign: "left" }}>
              <div style={{ fontSize: "11px", fontWeight: 700, color: "#5a6090", textTransform: "uppercase", marginBottom: 8 }}>Weakest Topics</div>
              {weakTopics.slice(0, 5).map((t, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "6px 10px", background: "#0d0f20", border: "0.5px solid #1e2245", borderRadius: "8px", marginBottom: "4px" }}>
                  <span style={{ fontSize: "12px", color: "#c5c9e8" }}>{t.topic}</span>
                  <span style={{ fontSize: "11px", color: "#ef5350" }}>{t.lapseRate} lapse rate</span>
                </div>
              ))}
            </div>
          )}
  ...
  return (
    <div style={fullscreenStyle}>
      <div style={{ display: "flex", alignItems: "center", gap: "10px", padding: "12px 52px 12px 20px", background: "#0a0c1e", borderBottom: "0.5px solid #1e2245", flexShrink: 0 }}>
        <Target size={16} style={{ color: "#ff7043" }} />
        <span style={{ fontSize: "13px", fontWeight: 600, color: "#ff7043", flex: 1 }}>Adaptive Drill</span>
        {combo.combo >= 3 && (
      ...
      <div style={{ padding: "12px 20px 0", flexShrink: 0 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "4px" }}>
          ...
          {q._topic && <span style={{ fontSize: "11px", color: "#ef5350", fontWeight: 600 }}>{q._topic}</span>}
        </div>
        <div style={{ height: "4px", background: "#0a0c1e", borderRadius: "4px", overflow: "hidden" }}>
          <div style={{ height: "100%", width: `${progressPct}%`, background: "linear-gradient(90deg, #ff7043, #FFD700)", borderRadius: "4px", transition: "width 0.3s ease" }} />
        </div>
      </div>

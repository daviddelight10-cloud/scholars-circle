...
export default function VoiceOrb({ state, micLevel = 0, onClick, size = 200 }) {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      if (s === VOICE_STATES.CONNECTING || s === VOICE_STATES.THINKING) {
        ...
        drawRipples(ctx, cx, cy, baseRadius, t, ml, color);
        drawCore(ctx, cx, cy, baseRadius * (1 + ml * 0.15), t, color);
      } else if (s === VOICE_STATES.SPEAKING) {
        drawWaveform(ctx, cx, cy, baseRadius, t, color);
        ...
        drawIdlePulse(ctx, cx, cy, baseRadius, t, color);
      }
...
function drawIdlePulse(ctx, cx, cy, r, t, color) {
...
function drawRipples(ctx, cx, cy, r, t, micLevel, color) {
  for (let i = 0; i < 3; i++) {
    const phase = (t * 0.8 + i * 0.33) % 1;
    const rippleR = r + phase * r * 2.5;
    const alpha = (1 - phase) * 0.4 * (0.5 + micLevel);
...
function drawWaveform(ctx, cx, cy, r, t, color) {
  const bars = 24;
  const maxBar = r * 0.6;
  ...
  for (let i = 0; i < bars; i++) {
    ...
    const barH = (Math.abs(wave) + 0.1) * maxBar;
    const innerR = r * 1.1;

...
import {
  Home as HomeIcon, BookOpen, Bot, BarChart3, Search, Menu, X, Folder,
...
function App() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (session) {
          ...
          try {
            ...
            // If profile fetch failed for other reasons (e.g. network error), try localStorage fallback
            if (!profileErr.message?.includes("profile") && !profileErr.message?.includes("not found") && !profileErr.message?.includes("403")) {
              try {
                const saved = JSON.parse(localStorage.getItem("scholars-circle-auth") || "{}");
                if (saved?.authUser) {
                  setAuth((a) => ({ ...a, user: saved.authUser, error: "", info: "" }));
                  if (saved.authToken) setToken(saved.authToken);
                  console.log("[restoreSession] Restored user from localStorage (backend unreachable)");
                }
              } catch (localErr) {
                console.error("[restoreSession] localStorage fallback failed:", localErr);
              }
            }
        ...
        // Offline fallback: restore user from localStorage
        try {
          const saved = JSON.parse(localStorage.getItem("scholars-circle-auth") || "{}");
          if (saved?.authUser) {
            setAuth((a) => ({ ...a, user: saved.authUser, error: "", info: "" }));
            if (saved.authToken) setToken(saved.authToken);
            console.log("[restoreSession] Restored user from localStorage (offline mode)");
          }
        } catch (localErr) {
          console.error("[restoreSession] localStorage fallback failed:", localErr);
        }
    ...
    // Loading overlay is hidden after auth restore completes (see effect below)

  }, []);



  // Hide loading overlay only after boot completes AND auth session is restored
  useEffect(() => {
  ...
  async function logout() {
    ...
    // Await this so the stale session token is removed from localStorage
    // BEFORE we reload the page below — otherwise the old session could
    ...
    // 6. Force a full page reload. This guarantees any per-user data cached
    // in a child component's own internal state (not covered by
    // clearUserState above) cannot leak into the next user's session on the
    // same device/tab — critical for shared/kiosk devices.

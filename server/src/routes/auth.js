...
router.post("/profile", requireSupabaseAuth, async (req, res) => {
  ...
  try {
    ...
    // Use provided username, or derive from fullName / email prefix
    const derivedUsername = (data.username || data.fullName || email.split("@")[0] || "scholar")
      .trim()
      .split(/\s+/)[0]
      .slice(0, 30);
    ...
    const user = await prisma.user.create({
      ...
      data: {
        ...
        fullName: data.username || data.fullName || null,
        role: desiredRole,
        activationKey,
    ...
    // Create UserProgress so XP, streaks, badges, and leaderboard work immediately
    try {
...
// PATCH /auth/username — Update current user's username
router.patch("/username", requireAuth, async (req, res) => {
  try {
    ...
    invalidateUserCache(req.user.supabaseId);
    return res.json({
      id: updated.id,
      email: updated.email,

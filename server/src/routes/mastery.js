...
router.post("/session", requireAuth, async (req, res) => {
  try {
    ...
    // Keep backward-compatible JSON blob in UserProgress (upsert handles missing record)
    const progress = await prisma.userProgress.upsert({
      where: { userId },
      create: {
        userId,
        mastery: { [subjectId]: masteryPct },
      },
      update: {},
    });

    // Merge mastery into existing JSON if record already existed
    const masteryJson = typeof progress.mastery === "object" ? progress.mastery : {};
    const alreadyBonused = masteryJson?.[subjectId] === 100;
    ...
    let xpBonus = 0;
    if (justMastered && !alreadyBonused) {
      xpBonus = 500;
      await prisma.userProgress.update({
        where: { userId },
